...
# IR reliability bounds — three panels from the 560 sim-anchored battery cells.
# A: net amplitude ebar (signed)  vs S~*Nch  -> |ebar| <= s everywhere; envelope +-0.08 x^-0.2
# B: anisotropy s                 vs S~*Nch  -> envelope 0.25 x^-0.2, floor 0.045
# C: worst-parameter |bias|/SE    vs Nch     -> envelope max(0.8/sqrt(N), 0.04); open = CI includes 0
suppressPackageStartupMessages({library(dplyr);library(tidyr);library(ggplot2);library(patchwork)})
d <- read.csv("digest/ir_collapse_cells.csv") |> filter(anchor=="sim") |>
  mutate(S=0.1*z, x=S*nch,
...
EA <- function(x) pmax(0.08*x^-0.2, 0.02)
EB <- function(x) pmax(0.25*x^-0.2, 0.045)
EC <- function(n) pmax(0.8/sqrt(n), 0.04)
cat("A violations |ebar| > envelope:", sum(abs(d$ebar) > EA(d$x)), "of", nrow(d), "\n")
cat("A subordination |ebar| > aniso :", sum(abs(d$ebar) > d$aniso), "of", nrow(d), "\n")
cat("B violations  aniso > envelope:", sum(d$aniso   > EB(d$x)), "of", nrow(d), "\n")
cat("C violations SIGNIFICANT zmax > envelope:", nrow(dc |> filter(sig, zmax_ci > EC(nch))),
    "of", sum(dc$sig), "significant cells\n")
...
pA <- ggplot(d, aes(x, ebar, colour=dlab)) +
  ...
  labs(title="A   net amplitude: subordinate", x=expression(tilde(S)%.%N[ch]),
       y=expression(bar(e)==mean~log~lambda)) + thm
...
pB <- ggplot(d, aes(x, aniso, colour=dlab)) +
  ...
  annotate("text", x=3, y=0.30, label="slope -1/5", size=2.4, colour="grey30", hjust=0) +
  labs(title="B   anisotropy, bounded by the product",
       x=expression(tilde(S)%.%N[ch]), y=expression(s==sd~log~lambda)) + thm
...
pC <- ggplot(dc, aes(nch, zmax_ci)) +
  ...
  labs(title="C   bias, bounded by channels alone",
       x=expression(N[ch]), y=expression(max[p]~"|bias|"/SE)) + thm

g <- pA + pB + pC + plot_layout(widths=c(1,1.15,1.15))
ggsave("figures/Figure_IR_reliability.pdf", g, width=7.0, height=2.7)
ggsave("figures/Figure_IR_reliability.png", g, width=7.0, height=2.7, dpi=170)
cat("written figures/Figure_IR_reliability.{pdf,png}\n")
