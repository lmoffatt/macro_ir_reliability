#!/bin/bash
# Dispatch figure_3_mle_LSE (the nonlinearsqr / classical-LSE MLE validation) across
...
#   projects/eLife_2025/ops/slurm/dispatch_figure_3_LSE.sh <cluster>     # e.g. example
# Tunables via env: NCHS and N_SIMS (parallel arrays, same length), N_NOISE,
# N_ALGO (default nonlinearsqr), GROUP_SIZE (a DSL axis — broadcast within one job;
...
#     projects/eLife_2025/ops/slurm/dispatch_figure_3_LSE.sh example
#   # chain after a build job:  DEPEND=<jobid> projects/eLife_2025/ops/slurm/dispatch_figure_3_LSE.sh example

set -eo pipefail

HERE="$(dirname "$(readlink -f "$0")")"          # .../eLife_2025/ops/slurm
WRAPPER="$HERE/run_macroir.sh"
SCRIPT="$HERE/../local/figure_3_mle_LSE.macroir"
...
for i in "${!NCHS[@]}"; do
    ...
    fp_arg=$(  printf -- '--filepath = "figures/data/figure_3_LSE_nch_%s_nsim_%s_%s_noise_%s"' "$nch" "$nsim" "$algo" "$nnoise")
    ...
    echo "submitted fig3LSE_nch_${nch}  job=${jobid}  n_sim=${nsim}  algo=${algo}  gs_axis=[${GROUP_SIZE[*]}]  -> ${WORKDIR}/figures/data/figure_3_LSE_nch_${nch}_nsim_${nsim}_${algo}_noise_${nnoise}_*"
done
done
