...
# dispatch_figure_3_G.sh: same cluster-profile sourcing, run_macroir.sh wrapper,
# printf injection idiom, and MACRODR_AXIS_SERIAL=1. Differences from _G: targets
# figure_3_mle_LSE.macroir (Stages 1-2 only, cloud + pool), and injects
# algo_family_approximation (=2 for nonlinearsqr) so the .macroir calls
# build_likelihood_function_with_family.
#
# FILE CONTRACT — figure_3_mle_LSE.macroir must NOT define the injected names itself
...
# (incl. algo_family_approximation), axis_interval, exp_n_step_/exp_n_samp_,
# group_size_axis, group_size, n_bootstrap_samples, min_groups_for_bootstrap, gn_max_iter.
# axis_h_fim, h_rel_value (the numerical-Fisher step) ARE injected here: that is the
# whole point of this dispatcher and the ONLY difference from dispatch_figure_3_LSE.sh.
# It exists because no LSE run in the freeze carries Likelihood_Numerical_Fisher_Information
# with values: the _G sweep declares the component and emits zero value-rows, and 433ed13,
# the only directory with a populated numerical Fisher, has no nonlinearsqr at all
# (verified 2026-08-12 over figures/data).
#
# Prereq: a built binary for the cluster:
#   projects/eLife_2025/ops/build_cluster.sh <cluster>
#
# Usage (from the repo base):
#   projects/eLife_2025/ops/slurm/dispatch_figure_3_LSE_numfim.sh <cluster>     # e.g. example
# Tunables via env: NCHS and N_SIMS (parallel arrays, same length), N_NOISE,
# N_ALGO (default nonlinearsqr), H_RELS, GROUP_SIZE (a DSL axis — broadcast within one job;
# cloud CSV gains a group_size column), N_BOOT, MIN_GROUPS, GN_MAX_ITER, CPUS, MEM,
# TIME, PARTITION, BIN, DEPEND (job id to wait for — unset = no dependency). Example:
#   NCHS="10000" N_SIMS="256" N_ALGO="nonlinearsqr" GROUP_SIZE="1 10 100" \
#     projects/eLife_2025/ops/slurm/dispatch_figure_3_LSE_numfim.sh example
#   # chain after a build job:  DEPEND=<jobid> projects/eLife_2025/ops/slurm/dispatch_figure_3_LSE_numfim.sh example

set -eo pipefail

HERE="$(dirname "$(readlink -f "$0")")"          # .../eLife_2025/ops/slurm
WRAPPER="$HERE/run_macroir.sh"
SCRIPT="$HERE/../local/figure_3_mle_LSE_numfim.macroir"
...
echo "[dispatch] commit=${commit}  run=${run}  WORKDIR=${WORKDIR}  (LSE / nonlinearsqr, stages 1-2 only)"
...
for i in "${!NCHS[@]}"; do
    ...
    # Map the algorithm label to its (recursive, averaging, taylor, micro, family) flags.
    # family: 0=macro, 1=micro, 2=nonlinearsqr (LSE). This dispatcher targets LSE; the
    # macro/micro arms are kept so a mixed N_ALGO still resolves — but note this .macroir
    # FIXES unitary_current + Current_Noise, which is only correct for the LSE path.
    case "$algo" in
        ...
        micro_R)      recursive=true;  averaging=0 ; taylor=false ; micro=true  ; family=1 ;;
    ...
    fp_arg=$(  printf -- '--filepath = "figures/data/figure_3_LSE_numfim_nch_%s_nsim_%s_%s_noise_%s"' "$nch" "$nsim" "$algo" "$nnoise")
    ...
    echo "submitted fig3LSEnumfim_nch_${nch}  job=${jobid}  n_sim=${nsim}  algo=${algo}  gs_axis=[${GROUP_SIZE[*]}]  -> ${WORKDIR}/figures/data/figure_3_LSE_numfim_nch_${nch}_nsim_${nsim}_${algo}_noise_${nnoise}_*"
done
done
